...
public final class WebSocketClient: Sendable {
    ...
    @available(macOS 13, *)
    public func connect(to endpoint: String, config: LCLWebSocket.Configuration) -> EventLoopFuture<WebSocket> {
        ...
        let upgradeResult = ClientBootstrap(group: eventloopGroup)
            ...
            .connect(host: host, port: port).flatMap { channel in
                ...
                let upgrader = NIOTypedWebSocketClientUpgrader<WebSocketUpgradeResult>(maxFrameSize: config.maxFrameSize) { channel, _ in
                    // TODO: probably need to decode the response from server to populate for more fields like extension
                    channel.eventLoop.makeCompletedFuture {
                        return WebSocketUpgradeResult.websocket(channel)
                    }
                }
                ...
                let httpRequestHead = HTTPRequestHead(version: .http1_1, method: .GET, uri: uri, headers: httpHeaders)
            
                
                let upgradeConfig = NIOTypedHTTPClientUpgradeConfiguration(upgradeRequestHead: httpRequestHead, upgraders: [upgrader]) { channel in
                    channel.eventLoop.makeCompletedFuture { .notUpgraded }
                }
                
                do {
                    return try channel.pipeline.syncOperations.configureUpgradableHTTPClientPipeline(configuration: .init(upgradeConfiguration: upgradeConfig))
                } catch {
        ...
        return upgradeResult.flatMapResult { upgradeResult in
            ...
            case .websocket(let channel):
                ...
                let websocket = WebSocket(channel: channel, type: .client, configuration: config, connectionInfo: websocketConnectionInfo)
                do {
                    try channel.pipeline.syncOperations.addHandlers([
                        NIOWebSocketFrameAggregator(minNonFinalFragmentSize: config.minNonFinalFragmentSize, maxAccumulatedFrameCount: config.maxAccumulatedFrameCount, maxAccumulatedFrameSize: config.maxAccumulatedFrameSize),
